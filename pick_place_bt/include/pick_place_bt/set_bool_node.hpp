...
{
public:
    explicit SetBoolService(const std::string& name, const BT::NodeConfig& conf, const BT::RosNodeParams& params) : RosServiceNode<SetBool>(name, conf, params)
    {}
...
public:
    explicit SetRobotBoolService(const std::string& name, const BT::NodeConfig& conf,
                                const rclcpp::Node::SharedPtr& node, const std::string& port_name)
